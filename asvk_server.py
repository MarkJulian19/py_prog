...
# def get_eth0_ip():  # Получает eth0 ip компьютера
#     print(netifaces.interfaces())
#     if "eth0" in netifaces.interfaces():
#         return netifaces.ifaddresses("eth0")[netifaces.AF_INET][0]['addr']
#     else:
#         return "ERROR: eth0 not found"
...
def is_correct(input_str):
    pattern_for_input_str = r"[Вв]ход[\d]+:\{\[(?:\['[A-Z]','[A-Z]'\],)*\['[A-Z]','[A-Z]'\]\],\{(?:'[A-Z]':[\d]+,)*'[A-Z]':[\d]+\}\}"
    ...
    def listen_user(self, user):
        ...
        while is_work:
            ...
            try:
                ...
                self.send_msg(user, "DATA IS GETED")
                print("DATA IS GETED")
...
# Находим апи eth0, если нашли то запускаем на нем наш сервер
eth0_ip = "0.0.0.0"
# eth0_ip = get_eth0_ip()
# if eth0_ip != "ERROR: eth0 not found":
#     Server(eth0_ip, 7777).start_server()
# else:
#     print("ERROR: eth0 not found")
Server(eth0_ip, 7777).start_server()